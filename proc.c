...
allocThread(struct proc *p)
{
  ...
  //check if there is a thread that not used, if there isn't return 
  acquire(&p->lock);
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if(p->state == ZOMBIE){
        ...
        for(t = p->pthreads; t < &p->pthreads[NTHREAD]; t++){
          ...
          panic("in wait, t is alive when p is dead"); 
        } 
        freevm(p->pgdir);
        p->state = UNUSED;
        p->pid = 0;
...
sleep(void *chan, struct spinlock *lk)
{
  ...
  t->chan = chan;
  t->state = SLEEPING;
  //cprintf("sleeping in before sched\n");

  sched();
  //cprintf("sleeping in after sched\n");
  // Tidy up.
  t->chan = 0;
...
procdump(void)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    ...
    for(t = p->pthreads; t < &p->pthreads[NTHREAD]; t++){
      if(t->state == UNUSED)
        continue;
      if(t->state >= UNUSED && t->state < NELEM(states) && states[t->state])
